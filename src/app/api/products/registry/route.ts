  ...
  try{await ensureSchema();const q=sql();
    await q`INSERT INTO paint_products(product_key,display_name,supplier,size,ean,item_no,source_name,area,subgroup,lookup_status,aliases,first_seen_at,last_seen_at,report_count,updated_at)
      ...
      ON CONFLICT(product_key) DO UPDATE SET
        ...
        ean=COALESCE(excluded.ean,paint_products.ean),
        item_no=COALESCE(excluded.item_no,paint_products.item_no),
...
export async function POST(req:Request){
  ...
  try{
    ...
    const rows=await q`SELECT product_key,ean,item_no,source_name,display_name,supplier,size,area,subgroup FROM paint_products WHERE product_key=${productKey} AND merged_into IS NULL LIMIT 1`;
    if(!rows.length)return NextResponse.json({error:'Produktet finnes ikke'},{status:404});
    const p:any=rows[0];
    const shouldPersist=action==='confirm_lookup';
    const result=await findObsbyggImage({productKey:p.product_key,ean:p.ean||'',itemNo:p.item_no||'',productName:p.display_name||p.source_name||'Ukjent produkt',rawName:p.source_name||'',supplier:p.supplier||'Ukjent',size:p.size||'',area:p.area||'',subgroup:p.subgroup||''},{force:true,persist:shouldPersist,exactOnly:true});
    if(!result.found)return NextResponse.json({ok:true,found:false,product:null});
    if(!shouldPersist)return NextResponse.json({ok:true,found:true,product:{product_key:productKey,ean:p.ean,item_no:p.item_no,source_name:p.source_name,website_name:result.websiteName||result.displayName,display_name:result.displayName||p.display_name||p.source_name,image_url:result.imageUrl||null,product_url:result.url||null,area:p.area,subgroup:result.subgroup||p.subgroup,lookup_status:'preview',lookup_method:result.matchMethod,matched_identifier:result.matchedIdentifier,match_confidence:result.confidence}});
    const updated=await q`SELECT product_key,ean,item_no,source_name,website_name,display_name,supplier,size,image_url,product_url,area,subgroup,lookup_status,report_count,lookup_method,matched_identifier,match_confidence,review_reason FROM paint_products WHERE product_key=${productKey}`;
