...
type SyncItem={productKey:string;productName:string;rawName?:string;supplier:string;size?:string;ean?:string;itemNo?:string;area?:string;subgroup?:string};
...
export async function POST(req:Request){
 ...
 try{
  ...
  for(const row of rows){
   ...
   if(!unique.has(key))unique.set(key,{productKey:key,productName:row.product,rawName:row.rawName,supplier:row.supplier,size:row.size,ean:row.ean,itemNo:row.itemNo,area:row.area,subgroup:row.subgroup});
  }
