...
export async function POST(req:Request){
  ...
  try{
    ...
    const imported=selected.length-replaced;
    if(selected.length){
      await q`INSERT INTO paint_reports(report_date,source_name,blob_url,report_data,updated_at)
        ...
        FROM jsonb_to_recordset(${JSON.stringify(selected)}::jsonb) AS x(report_date text,source_name text,blob_url text,report_data jsonb)
    ...
    return NextResponse.json({ok:true,imported,replaced,skipped,total:reports.length,firstDate:reports[0]?.date,lastDate:reports.at(-1)?.date});
  }catch(e){
