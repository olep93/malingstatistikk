...
export async function POST(req:Request){
  ...
  try{
    ...
    const sourceType=body?.sourceType==='powerbi'?'powerbi':'lumira';
    const importMode=body?.importMode==='daily'?'daily':'historical';
    ...
    const q=sql();
    const rows=await q`INSERT INTO paint_import_jobs(source_name,source_type,import_mode,status,created_by,blob_url,blob_size) VALUES(${sourceName},${sourceType},${importMode},'uploaded',${session.username},${blobUrl},${blobSize}) RETURNING id::text`;
