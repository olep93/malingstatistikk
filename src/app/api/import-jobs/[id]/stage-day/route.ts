...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
 ...
 try{
  ...
  for(const row of report.rows){
   ...
   if(!products.has(key))products.set(key,{product_key:key,product_data:{productKey:key,productName:row.product,rawName:row.rawName,supplier:row.supplier,size:row.size,ean:row.ean||row.itemNo,area:row.area,subgroup:row.subgroup}});
  }
