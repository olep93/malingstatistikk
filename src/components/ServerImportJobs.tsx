 ...
 const analyze=async(id:string,sourceName:string)=>{
  ...
  try{
   ...
   for(let i=0;i<reports.length;i++){
    setStatus({type:'working',text:`Lagrer rapportdag ${i+1} av ${reports.length} på serveren … Du kan fortsette fra mobil når dette steget er ferdig.`});
    let ok=false,last='';
    for(let attempt=0;attempt<3&&!ok;attempt++){
     const r=await fetch(`/api/import-jobs/${id}/stage-day`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({report:reports[i]})}),j=await json(r);
     if(r.ok)ok=true;else{last=j.error||'Lagring feilet';await new Promise(res=>setTimeout(res,800*(attempt+1)));}
    }
    if(!ok)throw new Error(`${last} (rapportdag ${i+1})`);
    if(i%5===0)await load();
   }
   setStatus({type:'success',text:`Analysen er ferdig: ${reports.length} rapportdager er lagret server-wide. Du kan nå synkronisere produkter her eller senere fra mobilen.`});
   await load();
