...
import {AlertCircle,CalendarDays,CheckCircle2,Database,Download,ExternalLink,LoaderCircle,Play,RefreshCw,Search,Trash2,UploadCloud} from 'lucide-react';
...
export default function ServerImportJobs({isAdmin,onImported,biReportUrl}:{isAdmin:boolean;onImported:()=>Promise<void>;biReportUrl?:string}){
 ...
 return <section className="panel serverImportJobs">
  <div className="panelHead"><div><span className="eyebrow">POWER BI-IMPORT</span><h2>Nasjonal periodeimport</h2><p className="panelIntro">Dette er det ordinære opplastingsløpet. Eksporten kan inneholde flere datoer og alle Obs BYGG-varehus.</p></div><button className="secondary" onClick={load}><RefreshCw size={16}/>Oppdater</button></div>
  <div className="importSourceGrid single">{uploadCard('powerbi','Last opp eksport fra Power BI','Velg Excel-eksporten med dato, varehus og EAN. Eksisterende rapportdager erstattes med de nasjonale tallene.')}</div>
