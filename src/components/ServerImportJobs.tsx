...
export default function ServerImportJobs({isAdmin,onImported}:{isAdmin:boolean;onImported:()=>Promise<void>}){
 ...
 const analyze=async(job:any)=>{
  ...
  try{
   ...
   setStatus({type:'success',text:`Analysen er ferdig: ${reports.length} rapportdager er lagret i små batcher. Neste steg er produktsynk og deretter import til hoveddatabasen.`});await load();
