...
export default function ServerImportJobs({isAdmin,onImported}:{isAdmin:boolean;onImported:()=>Promise<void>}){
 ...
 const analyze=async(job:any)=>{
  ...
  try{
   ...
   for(let i=0;i<remaining.length;i++){
    const completed=stagedDates.size+i+1;setStatus({type:'working',text:`Lagrer rapportdag ${completed} av ${reports.length}: ${remaining[i].date}. Tidligere lagrede dager hoppes over.`});
    let ok=false,last='';for(let attempt=0;attempt<3&&!ok;attempt++){const r=await fetch(`/api/import-jobs/${job.id}/stage-day`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({report:remaining[i]})}),j=await json(r);if(r.ok)ok=true;else{last=j.error||'Lagring feilet';await new Promise(res=>setTimeout(res,800*(attempt+1)))}}
    if(!ok)throw new Error(`${last} (${remaining[i].date})`);if(i%3===0)await load();
   }
   setStatus({type:'success',text:`Analysen er ferdig: ${reports.length} rapportdager er lagret. Neste steg er produktsynk og deretter import til hoveddatabasen.`});await load();
  }catch(e){setStatus({type:'error',text:`${e instanceof Error?e.message:'Analysen feilet'} Fremdriften som allerede er lagret, beholdes.`})}finally{try{await wakeLock?.release?.()}catch{}setBusy('')}
 };
 const run=async(id:string,mode:'sync-next'|'import-next')=>{setBusy(`${mode}-${id}`);let processed=0;try{for(let i=0;i<100000;i++){const r=await fetch(`/api/import-jobs/${id}/${mode}`,{method:'POST'}),j=await json(r);if(!r.ok)throw new Error(j.date?`${j.error} (${j.date})`:j.error);if(j.done)break;processed++;setStatus({type:'working',text:mode==='sync-next'?`Synkroniserer produkter · ${processed} puljer behandlet.`:`Importerer ${j.date||'rapportdag'} · ${j.rowCount||0} varelinjer kontrollert.`});if(i%5===0)await load()}setStatus({type:'success',text:mode==='sync-next'?'Produktsynkroniseringen er ferdig. Rapportdagene kan nå importeres.':`Import fullført: ${processed} rapportdager ble skrevet til hoveddatabasen.`});await load();if(mode==='import-next')await onImported()}catch(e){setStatus({type:'error',text:`${e instanceof Error?e.message:'Operasjonen stoppet.'} Fremdriften er lagret og kan fortsettes.`})}finally{setBusy('')}};
 ...
 return <section className="panel serverImportJobs">
  ...
  <div className="serverJobList">{jobs.map(job=>{const uploaded=['uploaded','analysis_error'].includes(job.status);const analyzed=!uploaded&&job.status!=='analyzing';const productsDone=job.total_products>0&&job.synced_products>=job.total_products;const importDone=job.total_days>0&&job.imported_days>=job.total_days;return <article key={job.id} className="serverJobCard"><header><div><b>{job.source_name}</b><span><strong>{sourceLabel(job.source_type)}</strong> · Opprettet av {job.created_by||'ukjent'}{job.blob_size?` · ${(Number(job.blob_size)/1024/1024).toFixed(1)} MB`:''}</span></div><small>{statusLabel(job.status)}</small></header><div className="serverJobSteps"><div className="jobStep done"><span>1</span><div><b>Fil lagret</b><small>Klar på alle enheter</small></div></div><div className={`jobStep ${analyzed?'done':job.status==='analyzing'?'active':''}`}><span>2</span><div><b>Analysert</b><small>{job.staged_days}/{job.total_days||'–'} rapportdager</small></div></div><div className={`jobStep ${productsDone?'done':analyzed?'active':''}`}><span>3</span><div><b>Produkter</b><small>{job.synced_products}/{job.total_products||'–'} synkronisert</small></div></div><div className={`jobStep ${importDone?'done':productsDone?'active':''}`}><span>4</span><div><b>Hoveddatabase</b><small>{job.imported_days}/{job.total_days||'–'} dager importert</small></div></div></div><div className="serverJobActions"><button className="primary" disabled={Boolean(busy)||!uploaded||!files[job.source_type==='powerbi'?'powerbi':'lumira']} onClick={()=>analyze(job)}>{busy===`analyze-${job.id}`?<LoaderCircle className="spin"/>:<Search size={15}/>}2. Analyser / fortsett</button><button className="secondary" disabled={Boolean(busy)||!analyzed||productsDone} onClick={()=>run(job.id,'sync-next')}><Play size={15}/>3. Synkroniser produkter</button><button className="successBtn" disabled={Boolean(busy)||!analyzed||!productsDone||importDone} onClick={()=>run(job.id,'import-next')}><CheckCircle2 size={15}/>{Number(job.imported_days||0)>0?`Fortsett (${job.imported_days}/${job.total_days})`:'4. Importer rapportdager'}</button>{isAdmin&&<button className="dangerBtn iconOnly" onClick={()=>remove(job.id)}><Trash2 size={16}/></button>}</div></article>})}{!jobs.length&&<div className="empty"><CalendarDays/><p>Ingen serverlagrede importjobber.</p></div>}</div>
 </section>
}
