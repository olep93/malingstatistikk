...
export default function ServerImportJobs({isAdmin,onImported}:{isAdmin:boolean;onImported:()=>Promise<void>}){
 ...
 const run=async(id:string,mode:'sync-next'|'import-next')=>{setBusy(`${mode}-${id}`);let processed=0;try{for(let i=0;i<100000;i++){const r=await fetch(`/api/import-jobs/${id}/${mode}`,{method:'POST'}),j=await json(r);if(!r.ok)throw new Error(j.date?`${j.error} (${j.date})`:j.error);if(j.done)break;processed+=mode==='sync-next'?Number(j.processed||0):1;setStatus({type:'working',text:mode==='sync-next'?`Beriker bare nye og mangelfulle produkter · ${processed} produkter behandlet${j.remaining!==undefined?` · ${j.remaining} gjenstår`:''}.`:`Importerer ${j.date||'rapportdag'} · ${j.rowCount||0} varelinjer kontrollert.`});if(i%5===0)await load()}setStatus({type:'success',text:mode==='sync-next'?'Produktberikelsen er ferdig. Navn og bilder oppdateres automatisk også i historikken.':`Import fullført: ${processed} rapportdager ble skrevet til hoveddatabasen.`});await load();if(mode==='import-next')await onImported()}catch(e){setStatus({type:'error',text:`${e instanceof Error?e.message:'Operasjonen stoppet.'} Fremdriften er lagret og kan fortsettes.`})}finally{setBusy('')}};
