...
export async function refreshReportCache(from:string,to:string){
 const q=sql();
 await q`DELETE FROM paint_report_rows c USING paint_reports p
   WHERE c.report_date=p.report_date AND p.report_date BETWEEN ${from}::date AND ${to}::date
     AND c.source_updated_at<p.updated_at`;
 await q`INSERT INTO paint_report_rows(
 ...
 FROM paint_reports p CROSS JOIN LATERAL jsonb_array_elements(COALESCE(p.report_data->'rows','[]'::jsonb)) r
 WHERE p.report_date BETWEEN ${from}::date AND ${to}::date
   AND NOT EXISTS(SELECT 1 FROM paint_report_rows c WHERE c.report_date=p.report_date)
