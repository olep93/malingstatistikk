...
function exactMatch(url:string,html:string,input:Input){
 const ids=identifiersFromPage(url,html);const wanted=digits(input.ean);
 if(wanted&&ids.has(wanted))return {method:wanted.length>=12?'EXACT_EAN' as const:'EXACT_ITEM_NO' as const,identifier:wanted};
 return null;
}
async function persist(input:Input,result:Result){
 ...
 await q`INSERT INTO paint_products(product_key,display_name,source_name,website_name,supplier,size,ean,image_url,image_source,product_url,category,subgroup,image_approved,aliases,lookup_status,last_fetched_at,normalization_version,area,updated_at,lookup_method,matched_identifier,match_confidence,review_reason,audit_status)
 VALUES(${input.productKey},${suggested},${sourceName},${websiteName},${input.supplier},${input.size||null},${input.ean||null},${result.imageUrl||null},${result.source||result.url||'automatic'},${result.url||null},${result.category||null},${result.subgroup||input.subgroup||null},${Boolean(result.imageUrl)},${JSON.stringify([input.productName,input.rawName].filter(Boolean))}::jsonb,${result.found?'found':result.status||'not_found'},now(),${NORMALIZATION_VERSION},${input.area||null},now(),${result.matchMethod||'NONE'},${result.matchedIdentifier||null},${result.confidence||0},${result.found?null:'Ingen eksakt nummermatch på Obsbygg.no'},${result.found?'ok':'review'})
 ...
 ean=COALESCE(excluded.ean,paint_products.ean),aliases=(SELECT jsonb_agg(DISTINCT x) FROM jsonb_array_elements(paint_products.aliases || excluded.aliases) x),
