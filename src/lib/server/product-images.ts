...
type Result={found:boolean;imageUrl?:string;displayName?:string;websiteName?:string;url?:string;category?:string;source?:string;status?:string};
const cleanTitle=(title:string)=>decodeHtml(title).replace(/\s*[|–-]\s*Obs BYGG.*$/i,'').replace(/\s*\|\s*Coop.*$/i,'').trim();
const customerName=(title:string,fallback:string)=>cleanTitle(title||fallback).replace(/\s+interiørmaling\b/i,'').replace(/\s+eksteriørmaling\b/i,'').replace(/\s+terrassebeis\b/i,' Terrassebeis').replace(/\s+/g,' ').trim();
...
async function persist(input:Input,result:Result){
 ...
 const suggested=result.displayName||input.productName;
 await q`INSERT INTO paint_products(product_key,display_name,source_name,website_name,supplier,size,ean,image_url,image_source,product_url,category,image_approved,aliases,lookup_status,last_fetched_at,updated_at)
 VALUES(${input.productKey},${suggested},${sourceName},${websiteName},${input.supplier},${input.size||null},${input.ean||null},${result.imageUrl||null},${result.source||result.url||'automatic'},${result.url||null},${result.category||null},${Boolean(result.imageUrl)},${JSON.stringify([input.productName,input.rawName].filter(Boolean))}::jsonb,${result.found?'found':'not_found'},now(),now())
 ...
 product_url=COALESCE(excluded.product_url,paint_products.product_url),category=COALESCE(excluded.category,paint_products.category),image_approved=paint_products.image_approved OR excluded.image_approved,
 ean=COALESCE(excluded.ean,paint_products.ean),aliases=(SELECT jsonb_agg(DISTINCT x) FROM jsonb_array_elements(paint_products.aliases || excluded.aliases) x),lookup_status=excluded.lookup_status,last_fetched_at=now(),updated_at=now()`;
}
...
export async function findObsbyggImage(input:Input,opts:{force?:boolean}={}):Promise<Result>{
 ...
 const existing=await q`SELECT display_name,website_name,image_url,product_url,category,lookup_status,last_fetched_at FROM paint_products WHERE product_key=${input.productKey} LIMIT 1`;
 const row:any=existing[0];const stale=!row?.last_fetched_at||Date.now()-new Date(row.last_fetched_at).getTime()>90*86400000;
 if(row&&!opts.force&&!stale){return {found:row.lookup_status==='found',imageUrl:row.image_url,displayName:row.display_name,websiteName:row.website_name,url:row.product_url,category:row.category,source:'database',status:row.lookup_status};}
 const known=catalogEntry(input.productName,input.rawName);
 if(known?.image){const r={found:true,imageUrl:known.image,displayName:known.name,websiteName:known.name,url:known.pageUrl,category:known.category,source:'catalog',status:'found'};await persist(input,r);return r;}
 if(known?.pageUrl){const page=await fetchPage(known.pageUrl);if(page){const r={found:true,imageUrl:page.image,displayName:known.name,websiteName:page.title,url:known.pageUrl,category:known.category,source:'catalog-page',status:'found'};await persist(input,r);return r;}}
 ...
 for(const term of [...new Set(terms)])for(const searchUrl of [`https://www.obsbygg.no/sok?q=${encodeURIComponent(term)}`,`https://www.obsbygg.no/sok?query=${encodeURIComponent(term)}`]){
  try{const r=await fetch(searchUrl,{headers,cache:'no-store',redirect:'follow',signal:AbortSignal.timeout(14000)});if(!r.ok)continue;const html=await r.text();const candidates=[] as {url:string;image?:string;title:string;score:number}[];for(const url of productLinks(html)){const page=await fetchPage(url);if(page)candidates.push({url,image:page.image,title:page.title,score:score(page.title,input)});}candidates.sort((a,b)=>b.score-a.score);const best=candidates[0];if(best&&best.score>0){const websiteName=best.title;const result={found:true,imageUrl:best.image,displayName:customerName(websiteName,input.productName),websiteName,url:best.url,category:known?.category,source:'obsbygg-search',status:'found'};await persist(input,result);return result;}}catch{}
 }
 const result={found:false,displayName:known?.name||input.productName,websiteName:undefined,category:known?.category,source:'obsbygg-search',status:'not_found'};await persist(input,result);return result;
}
